...
__all__ = ['logger', 'nlp', 'get_training_dfs', 'postprocess_df', 'create_and_log_wandb_postrun_plots',
           'get_interesting_idx', 'print_stats', 'print_interesting_text_stats', 'misc_stats',
           'get_common_additions_and_removals']
...
# Cell
def get_training_dfs(cfg):
    """Return a dict of dataframes with all training and eval data"""
    df_d = dict()
    for key in cfg.splits + ['training_step']:  df_d[key] = pd.read_csv(f"{cfg.path_run}{key}.csv")
...
def _join_punctuation(seq):
    "Generator to join tokens respecting punctuation, but doesn't work that well."
    current = next(iter(seq))
    for nxt in seq:
        if nxt in set(string.punctuation):    current += nxt
        else:                  yield current; current = nxt
    yield current
